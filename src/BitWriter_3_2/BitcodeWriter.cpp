...
static inline uint64_t encodeLLVMAttributesForBitcode(llvm::AttributeSet _A,
                                                      unsigned i) {
  // A.Raw(i) doesn't support Dereferenceable attr, so strip it out.
  auto A = _A.removeAttribute(_A.getContext(), i, Attribute::Dereferenceable);
